...
const ResultsTable = ({ prospects }) => {
  const getBadgeClass = (opportunityType) => {
    ...
    if (opportunityType.includes('réseaux sociaux')) return 'badge-info';
        ...
        <tbody>
          {prospects.length === 0 ? (
            ...
            prospects.map((prospect, index) => {
              ...
              return (
                <tr key={index}>
                  <td className="company-name">{prospect.companyName}</td>
                  <td>{prospect.city || '-'}</td>
                  <td>{prospect.sector || '-'}</td>
                  <td>
                    {prospect.phone ? (
                      <a href={`tel:${prospect.phone}`} className="contact-link phone-link" title="Appeler">
                        📞 {prospect.phone}
                      </a>
                    ) : (
                      '-'
                  ...
                  <td>
                    {prospect.websiteUrl ? (
                      ...
                      >
                        🌐 {prospect.websiteUrl.length > 25
                          ? `${prospect.websiteUrl.substring(0, 25)}...`
                          : prospect.websiteUrl}
                      </a>
                    ) : (
                      '-'
